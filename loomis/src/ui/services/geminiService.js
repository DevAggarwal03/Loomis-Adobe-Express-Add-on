...
export async function analyzeDesignV5(base64Image, mimeType, userContext = null) {
  ...
  const ai = new GoogleGenAI({ apiKey: "AIzaSyCNCgAjNPrX74DAnkzYJUCQPoSsygiwM4g" });
...
export async function analyzeDesign(base64Image, mimeType) {
  const apiKey = process.env.GEMINI_API_KEY || "AIzaSyCNCgAjNPrX74DAnkzYJUCQPoSsygiwM4g";
  if (apiKey.length === 0) {
    throw new Error(
